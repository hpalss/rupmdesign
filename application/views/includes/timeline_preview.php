<?php

$message_row = false;
if (isset($is_message_row)) {
    $message_row = $is_message_row;
}

$box_class = "mb15";
$caption_class = "more";
$caption_lang = " " . lang('more');
if ($message_row) {
    $box_class = "message-images mb5 mt5";
    $caption_class .= " message-more";
    $caption_lang = "";
}

if ($files && count($files)) {
    $timeline_file_path = get_setting("timeline_file_path");
    $total_files = count($files);
    echo "<div class='timeline-images" . $box_class . "'>";
    $file_name = $files[0]['file_name'];
    $more_image = "";
    // if ($total_files > 1) {
    //     $more_count = $total_files - 1;
    //     $more_image = "<span class='$caption_class'>+" . $more_count . $caption_lang . "</span>";
    // }

    $shown_preview_image = false;
    $is_localhost = is_localhost();

    foreach ($files as $file) {
        $file_name = $file['file_name'];
        $file_id = get_array_value($file, "file_id");
        $service_type = get_array_value($file, "service_type");

        $url = get_source_url_of_file($file, $timeline_file_path);
        $thumbnail = get_source_url_of_file($file, $timeline_file_path, "thumbnail");

        $image = "";
        $actual_file_name = remove_file_prefix($file_name);
        echo '<div class"media"><div class="media-left">';
        if (is_viewable_video_file($file_name)) {

            if (!$shown_preview_image) {
                $image = "<img src='" . get_file_uri("assets/images/video_preview.jpg") . "' alt='video'/>$more_image";
                // $shown_preview_image = true;
            }
            echo "<a href='$url' data-title='" . $actual_file_name . "' class='media-object mfp-iframe'>$image</a>";
        } else if (is_viewable_image_file($file_name)) {

            if (!$shown_preview_image) {
                $image = "<img src='$thumbnail' alt='img'/>$more_image";
                // $shown_preview_image = true;
            }
            $mfp_class = "mfp-image";
            if ($file_id && $service_type == "google") {
                $mfp_class = "mfp-iframe";
            }
            echo "<a href='$url' class='$mfp_class media-object' data-title='" . $actual_file_name . "'>$image</a>";
        } else {
            if (!$shown_preview_image) {
                $image = "<div class='inline-block media-object'><div class='file-mockup'><i class='fa fa-" . get_file_icon(strtolower(pathinfo($file_name, PATHINFO_EXTENSION))) . "'></i></div></div>$more_image";
                // $shown_preview_image = true;
            }

            if ($file_id && $service_type == "google") {
                echo "<a data-viewer='google' href='$url' class='mfp-iframe' data-title='" . $actual_file_name . "'>$image</a>";
            } else {
                if (!$is_localhost && is_google_preview_available($file_name)) {
                    echo "<a data-viewer='google' href='https://drive.google.com/viewerng/viewer?url=$url?pid=explorer&efh=false&a=v&chrome=false&embedded=true' class='mfp-iframe' data-title='" . $actual_file_name . "'>$image</a>";
                } else {
                    $uid = uniqid(rand());
                    echo "<a href='#$uid' class='mfp-inline' data-title='" . $actual_file_name . "'>" . $image . "</a>" . '<div id="' . $uid . '" class="mfp-hide container max-w500 text-center p20 bg-white">' . lang("file_preview_is_not_available") . '<div class="text-off">' . $actual_file_name . '</div>' . '</div>';
                }
            }
        }
        echo "</div>";
        echo '<div class="media-body">
            <strong class="media-heading">' . substr($actual_file_name,0,15) . '</strong>
            <a href='.$url.' data-title="' . $actual_file_name . '" class="pull-right"><i class="fa fa-download" aria-hidden="true"></i></a>
        </div></div>';
    }
    echo "</div>";
}else{
    echo lang("no_attachments");
}