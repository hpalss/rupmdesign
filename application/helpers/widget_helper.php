...
/**
 * get open estimate widget
 * @param boolean $return_as_data
 * @return html
 */
if (!function_exists('open_estimates_widget')) {

    function open_estimates_widget($return_as_data = false) {
        $ci = get_instance();
        $view_data["count"] = $ci->Estimate_requests_model->getDetailsCount();
        return $ci->load->view("estimate_requests/open_estimates_widget", $view_data, $return_as_data);
    }
 ...
 * @return html
 */
if (!function_exists('task_widget')) {

    function task_widget($custom_class = "", $return_as_data = false) {
        $ci = get_instance();
        $info = $ci->Expenses_model->get_income_expenses_info();
        $view_data["income"] = $info->income ? $info->income : 0;
        $view_data["expenses"] = $info->expneses ? $info->expneses : 0;
        $view_data["custom_class"] = $custom_class;
        return $ci->load->view("projects/tasks/task_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('invoice_statistics_widget')) {
    ...
    function invoice_statistics_widget($return_as_data = false, $options = array()) {
        ...
        $view_data["currency_symbol"] = $currency_symbol ? $currency_symbol : get_setting("default_currency");
...
/**
 * get bidding count status widteg
 * @param integer $user_id
 * @param boolean $return_as_data
 * @return html
 */
if (!function_exists('count_bidding_widget')) {

    function count_bidding_widget($user_id = 0, $return_as_data = false) {
        $ci = get_instance();
        $bid_by = $user_id ? $user_id : $ci->login_user->id;
        $info = $ci->Developer_bid_model->getCount(NULL,$bid_by);
        $view_data["allBids"] = $info;
        $view_data["totalEarned"] = 0;
        $expense = $ci->Expenses_model->get_details(['user_id'=>$bid_by])->result();
        if($expense){
            foreach ($expense as $key => $val) {
                $tax = 0;
                $tax2 = 0;
                if ($val->tax_percentage) {
                    $tax = $val->amount * ($val->tax_percentage / 100);
                }
                if ($val->tax_percentage2) {
                    $tax2 = $val->amount * ($val->tax_percentage2 / 100);
                }
                $view_data["totalEarned"] += $val->amount + $tax + $tax2;
            }
        }
        $view_data["totalEarned"] = to_currency($view_data["totalEarned"]);
        return $ci->load->view("projects/widgets/bidding_status_widget", $view_data, $return_as_data);
    }
