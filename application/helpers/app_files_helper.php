    ...
    function upload_file_to_temp() {
        ...
        if (!empty($_FILES)) {
            ...
            if (get_setting("enable_google_drive_api_to_upload_file") && get_setting("google_drive_authorized")) {
                ...
                echo json_encode(["file_name"=>$file_name,"file_path"=>$target_file]);
                die;
                // echo $target_file;
            }
        }
...
if (!function_exists('get_source_url_of_file')) {
    ...
    function get_source_url_of_file($file_info = array(), $file_path = "", $view_type = "", $is_attachment = false) {
        ...
        if ($service_type == "google") {
            //google drive file
            return get_source_url_of_google_drive_file($file_id, $view_type);
        } else {
            //local file
            $full_file_path = $file_path . $file_name;
            return $is_attachment ? $full_file_path : get_file_uri($full_file_path);
        }
    }
...
if (!function_exists("get_source_url_of_google_drive_file")) {
    ...
    function get_source_url_of_google_drive_file($file_id = "", $view_type = "") {
        if ($view_type == "thumbnail") {
            return "https://drive.google.com/thumbnail?id=$file_id&sz=s700";
        } else {
            //preview
        ...
        if (is_array($files)) {
            foreach ($files as $file) {
                if (is_array($file)) {
                    ...
                    delete_file_from_directory($directory_path . array($file)); //single file
                }
            }
        } else {
