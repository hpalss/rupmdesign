...
if (!function_exists('get_notification_config')) {
    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $estimate_bidding_link = function($options) {
            $url = "";
            if (isset($options->estimate_request_id)) {
                $url = get_uri("bidding/view_estimate_request/" . $options->estimate_request_id);
            }
        ...
        $events = array(
            ...
            "admin_bid_received" => array(
                "notify_to" => array("client_primary_contact","team_members", "team"),
                "info" => $estimate_request_link
            ),
            "developer_bid_received" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_request_link
            ),
            "client_bid_received" => array(
                "notify_to" => array("client_primary_contact", "client_all_contacts", "team_members", "team"),
                "info" => $estimate_request_link
            ),
            "discussion_reply_to_admin" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_request_link
            ),
            "discussion_reply_to_client" => array(
                "notify_to" => array("client_primary_contact", "team"),
                "info" => $estimate_request_link
            ),
            "estimate_status_changed" => array(
                "notify_to" => array("client_primary_contact", "team","team_members"),
                "info" => $estimate_request_link
            ),
            "estimate_question_asked" => array(
                "notify_to" => array("client_primary_contact","team_members", "team"),
                "info" => $estimate_request_link
            ),
            "estimate_question_answered" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_bidding_link
            ),
            "estimate_requests" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_request_link
            ),
            "client_accepted_estimate" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_request_link
            ),
            "client_rejected_estimate" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $estimate_request_link
            ),
            "estimate_sent" => array(
...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        } else if ($notification->event == "estimate_sent") {
...
if (!function_exists('send_push_notifications')) {
    ...
    function send_push_notifications($event, $push_notify_to, $user_id = 0, $notification_id = 0) {
        ...
        if ($pusher_app_id && $pusher_key && $pusher_secret && $pusher_cluster) {
            ...
            $message = $to_user_name ? sprintf(lang("notification_" . $event), $to_user_name) : lang("notification_" . $event);
            ...
            if ($push_notify_to) {
                ...
                foreach ($push_notify_to_array as $user_id) {
                    $pusher->trigger('user_' . $user_id . '_channel', 'rise-pusher-event', $data);
                }
            }
        }
    }
