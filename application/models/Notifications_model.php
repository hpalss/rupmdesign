...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        //find estimate request cleint
        if (in_array("client_primary_contact", $notify_to_terms) && $estimate_request_id) {
            $where .= " OR ($users_table.client_id=(SELECT $estimate_request_table.client_id FROM $estimate_request_table WHERE $estimate_request_table.id=$estimate_request_id)) ";
        }
    ...
    function get_notifications($user_id, $offset = 0, $limit = 20) {
        ...
        $events_table = $this->db->dbprefix('events');
        $announcements_table = $this->db->dbprefix('announcements');
        $estimate_requests = $this->db->dbprefix('estimate_requests');
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $notifications_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name, $users_table.image AS user_image,
                 ...
                 $estimate_requests.title AS estimate_request_title,
                 $estimate_requests.category AS estimate_request_category,
        ...
        LEFT JOIN $estimate_requests ON $estimate_requests.id=$notifications_table.estimate_request_id
