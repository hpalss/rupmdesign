...
class Tasks_model extends Crud_model {
    ...
    function get_upcomming_task($limit=5) {
        $tasks_table = $this->db->dbprefix('tasks');
        $task_status_table = $this->db->dbprefix('task_status');
        $projects_table = $this->db->dbprefix('projects');
        $this->db->select("$tasks_table.*,$projects_table.title as project_title");
        $this->db->where("$tasks_table.deadline >", get_current_utc_time());
        $this->db->where("$tasks_table.status !=", "done");
        if (!$this->login_user->is_admin) {
            $this->db->where("$tasks_table.assigned_to", $this->login_user->id);
        }
        $this->db->order_by("$tasks_table.deadline", 'ASC');
        $this->db->join($projects_table, "$tasks_table.project_id = $projects_table.id", 'left');
        return $this->db->get($tasks_table, $limit)->result();
    }
    function get_overdue_task($limit=5) {
        $tasks_table = $this->db->dbprefix('tasks');
        $task_status_table = $this->db->dbprefix('task_status');
        $projects_table = $this->db->dbprefix('projects');
        $this->db->select("$tasks_table.*,$projects_table.title as project_title");
