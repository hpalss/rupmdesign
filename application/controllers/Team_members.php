...
class Team_members extends MY_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $projectCount = $this->Projects_model->count_project_status(['user_id'=>$data->id]);
        $count = 0;
        if($projectCount){
            $count = $projectCount->open;
        }
        $online = is_online($data->last_online);
        $user_avatar = "<span class='avatar avatar-xs'><img src='$image_url' alt='...'>$online</span>";
        $full_name = $data->first_name . " " . $data->last_name . " <span class='text-warning'>(".$count.")<span>";
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                // $this->load->model('developer_bidding');
                
                $view_data['tab'] = $tab; //selected tab
                $view_data['user_info'] = $user_info;
                $view_data['social_link'] = $this->Social_links_model->get_one($id);
                $view_data['estimate_requests'] = true;//$this->Estimate_requests_model->get_details(array("id" => $id))->result();
    ...
    public function estimate_requests($id='')
    {
        ...
        $view_data['biddingCounts'] = $this->Developer_bid_model->getDetailsCount($id);
        $this->load->view("team_members/partials/estimate_request",$view_data);
    }
    ...
    function download_file($id) {
        ...
        //serilize the path
        $file_data = serialize(array(array("file_name" => $file_info->file_name, "file_id" => $file_info->file_id)));
