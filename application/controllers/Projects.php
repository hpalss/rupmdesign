...
class Projects extends MY_Controller {
    ...
    private function show_assigned_tasks_only_user_id() {
        if ($this->login_user->user_type === "staff") {
    ...
    function save() {
        ...
        $data = array(
            ...
            "description" => str_replace("undefined<br>","",$this->input->post('description')),
            "client_id" => $this->input->post('client_id'),
    ...
    function all_tasks_calendar($id=NULL) {
        $view_data = array('id' => $id,"assign_to_dropdown"=>[] );
        $view_data['assigned_to_dropdown'] = $this->_get_project_members_dropdown_list();
        $this->template->rander("projects/tasks/calendar/index",$view_data);
    }
    public function get_tasks_states() {
        $upcommingTasks = $this->Tasks_model->get_upcomming_task();
        $overdueTasks = $this->Tasks_model->get_overdue_task();
        $this->load->view('projects/tasks/widget-body', ['overdue'=>$overdueTasks,'upcomming'=>$upcommingTasks]);
        // echo json_encode(["overdue"=>$overdueTasks,'upcomming'=>$upcommingTasks]);
    }
    public function task_data($id='',$status="")
    {
        $specific_user_id = $this->input->get('assign')?$this->input->get('assign'):$this->login_user->id;
        if (!$this->login_user->is_admin) {
            $specific_user_id = $this->login_user->id;
        }
        $options = array(
            "specific_user_id" => $specific_user_id,
            // "project_status" => "open",
            "status_ids" => $this->input->get('status') ? implode(",", $this->input->get('status')) : "",
    ...
    private function _initialize_all_related_data_of_project($project_id = 0) {
        //we have to check if any defined project exists, then go through with the project id
        if ($project_id) {
            $this->init_project_permission_checker($project_id);
    ...
    function task_modal_form() {
        ...
        $view_data = $this->_initialize_all_related_data_of_project($final_project_id);
    ...
    function download_file($id) {
        ...
        //serilize the path
        $file_data = serialize(array(array("file_name" => $file_info->project_id . "/" . $file_info->file_name, "file_id" => $file_info->file_id)));
        ...
        if ($files_ids) {
            ...
            if ($files) {
                ...
                foreach ($files->result() as $file_info) {
                    ...
                    $file_path_array[] = array("file_name" => $file_info->project_id . "/" . $file_info->file_name, "file_id" => $file_info->file_id);
                }
