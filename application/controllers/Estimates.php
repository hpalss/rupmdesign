...
class Estimates extends MY_Controller {
    ...
    function modal_form() {
        ...
        $estimate_request_info = $this->Estimate_requests_model->get_one($this->input->post('id'));
        if (!$estimate_request_id) {
            $estimate_request_id = $estimate_request_info->id;
        }
    ...
    private function _get_estimate_status_label($estimate_info, $return_html = true) {
        ...
        return status_label($estimate_info->status,$return_html);
