...
class Settings extends MY_Controller {
    ...
    function invoices() {
        $this->template->rander("settings/invoices");
    }

    function save_invoice_settings() {
        $settings = array("allow_partial_invoice_payment_from_clients", "invoice_color", "invoice_footer", "send_bcc_to", "invoice_prefix", "invoice_style", "invoice_logo", "send_invoice_due_pre_reminder", "send_invoice_due_after_reminder", "send_recurring_invoice_reminder_before_creation", "default_due_date_after_billing_date");
    ...
    public function notification_add_modal_form()
    {
        $this->load->view("settings/notifications/add_modal_form");
    }
    ...
    function add_notification_settings() {
        ...
        $data = array(
            "enable_web" => $this->input->post("enable_web")?1:0,
            "enable_email" => $this->input->post("enable_email")?1:0,
            "event" => $this->input->post("title"),
            "category" => $this->input->post("category"),
            "notify_to_terms" => $this->input->post("term"),
            "notify_to_team" => $this->input->post("notify_to_team")?1:0,
            "notify_to_team_members" => $this->input->post("notify_to_team_members")?1:0
        );


        $save_id = $this->Notification_settings_model->save($data);
    ...
    function save_module_settings() {
        ...
        $settings = array("module_timeline", "module_event", "module_todo", "module_note", "module_message", "module_chat", "module_invoice", "module_expense", "module_attendance", "module_leave", "module_estimate", "module_estimate_request", "module_ticket", "module_announcement", "module_project_timesheet", "module_help", "module_knowledge_base");
